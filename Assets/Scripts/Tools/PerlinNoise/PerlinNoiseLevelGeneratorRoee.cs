...
public class PerlinNoiseLevelGeneratorRoee : MonoBehaviour
{
   ...
   [SerializeField] GameObject[] TopCornerMidPrefabs = new GameObject[10];
   [SerializeField] int[] TopCornerMidPrefabsMaxGenerates = new int[10];
    ...
    private int[] StartTopCornerMidPrefabsMaxGenerates;
    private void Start()
    {
        CreateLevel();
    }
    ...
    {
        Debug.Log("AWAKE");
        Debug.Log("Start TopCornerMidPrefabsMaxGeneratesLength is: " + TopCornerMidPrefabsMaxGenerates.Length);
        StartTopCornerMidPrefabsMaxGenerates = TopCornerMidPrefabsMaxGenerates;

        TopCornerMidPrefabsMaxGenerates = StartTopCornerMidPrefabsMaxGenerates;

        Debug.Log("TopCornerMidPrefabsMaxGeneratesLength is: " + TopCornerMidPrefabsMaxGenerates.Length);
        ...
        for (int x = startXGenerating; x < stopXGenerating; x++)
        {
            for (int y = 0; y < platformHeight; y++)
            {
                ...
                else if (y == platformHeight - 1)//^
                {
                    ...
                    // 1 = goal
                    // 2 = platform
                    int randomPrefab = Random.Range(0, TopCornerMidPrefabs.Length);
              
                    if (TopCornerMidPrefabs[randomPrefab] != null && TopCornerMidPrefabsMaxGenerates[randomPrefab] > 0)
                    {
                        GameObject usedPrefab = TopCornerMidPrefabs[randomPrefab];
                        if (usedPrefab != null)
                        {
                            Instantiate(usedPrefab, new Vector3(x - 7.5f, y - 4.5f), Quaternion.identity, _lastGeneratedPlatform);
                            TopCornerMidPrefabsMaxGenerates[randomPrefab]--;
                        }
                    }
